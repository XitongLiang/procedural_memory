...
```
used_skill_ids 非空：
  过滤：只保留 managed_skills（学习模块创建的 skill）
  ├─ 过滤后非空 → 升级路线（§6），窗口消耗
  ├─ 过滤后为空（全是外部 skill）→ 跳过该窗口，不进入任何路线
  │    （外部 skill 的执行轨迹是按别人的流程走的，学出来是副本）
  └─
...
for skill_id in used_skill_ids:
  ...
  ① EvaluateSkill — 反馈修正检测（小模型，§6.2）
     逐条 user 消息 → 反馈有效性判断
     dissatisfied → 提取纠正内容 → 与 skill 步骤对应 → signal

  ② signal 为空 → 记录 EVALUATED(no_signal)，跳过

  ③ signal 非空 → EvolveSkill（大模型，§6.3）
...
SKILL.md 写入成功后，触发增量 scripts / references 更新（同 §8.3.2 并行生成，基于新版 SKILL.md diff 重新生成变动部分）。
...
#### 8.4.1 EnrichSkill（大模型）

直接将 Chunk 内容和已有 SKILL.md 交给大模型，一次完成差异分析和内容合并：

```
[System]
你是一个 workflow 内容合并专家。
对比新执行记录与已有 SKILL.md，判断是否有增量信息，有则合并使其更完整。
...
判断维度：
1. 有没有 skill 未覆盖的新触发场景？
2. agent 是否用了 skill 没提到的做法且成功？
3. skill description 是否需要扩展以覆盖此类场景？

只输出 JSON：
{
  "action": "enrich|no_change",
  "needs_description_improve": true|false,
  "content": "完整新 SKILL.md（action=enrich 时）",
  "added": ["新增了什么及原因"],
  "skipped": ["忽略了什么及原因"]
}

action=no_change 且 needs_description_improve=true → 仅进入 §8.4.2 改善 description
...
SKILL.md 写入成功后，触发增量 scripts / references 更新（同 §8.3.2）。

#### 8.4.2 Description 改善（小模型，可选）
...
#### 小模型决策 Prompt（0.50–0.85 区间，两层共用）

借鉴 AutoSkill 结构化决策 + XSkill 合并原则：
...
#### 合并执行 Prompt（score ≥ 0.85 自动触发 / 决策=merge 后触发）

借鉴 AutoSkill Skill Merger + XSkill MERGE_PROMPT：
...
降级路径：LLM 失败时取两边 description 中较长的一份（近期优先 — RECENCY BIAS），context 取并集去重。
