...
V6 的架构由三块基石组成，逐块展开在 §1–§3：

| # | 中心思想 | 核心产出 | 修的痛点 |
|---|---|---|---|
| 1 | **Task Trace 提取** | 每 chunk 1 次产出结构化 Markdown，下游共享 | 每个提取器各扫一遍原始对话、过程态和知识态混存、Step 识别不稳定 |
| 2 | **滑动窗口 + 主题拼接** | 长任务被自动拼成一个 chunk 一次性消费 | 单窗口提取碎片化、长任务主题跨窗口丢失、LLM token 成本线性 |
| 3 | **分类 + 候选池 + 快慢车道** | `TSE / Fb / Tp` 三元 + `task / sub-task` 双级锚点 + 单表候选池 + 快慢两条升格通路 | 程序记忆粒度单一、硬约束生效慢、软经验无证据验证、候选/活跃层向量空间不一致 |

Task Trace 是 V6 最独特的创新，放在最前面讲；滑动窗口负责为它准备干净的输入 chunk；分类/候选池/快慢车道是组织它产出的下游知识。三块形成"输入 → 中间层 → 输出管理"的完整闭环。

剩下的小优化（水位重传、向量 1:1 拷贝、embedding 一致性）都可以归到这三块的延伸里。
...
CREATE TABLE procedural_candidates (
  ...
  payload         TEXT,  -- fb/tp: description；tse: bullets_json
...
120s 定时器扫水位
  │
  ├─ 读 mem_conversation 水位之后的新 turn
  │
  ├─【中心思想 2：滑动窗口 + 主题拼接】
  ...
  └─ chunk flush:
       ...
       └─【中心思想 3：分类 + 候选池 + 快慢车道】
           ...
           │
           └─ 升格分车道
               is_hard=true → 快车道：在线立即升格到 active
...
> **V6 的核心是 Task Trace 作中间事件层把所有下游提取的成本摊到一次 LLM 调用；上游用滑动窗口 + 主题拼接喂它完整长任务；下游按粒度分类、按信号强度分车道更新候选池，让 active 层周期性追上主文件。**

三块基石都不独立创新，但**组合起来**第一次同时解决了以下 7 个传统痛点：
