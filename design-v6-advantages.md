...
| # | 中心思想 | 核心产出 | 修的痛点 | 这些痛点在哪些自演进系统中存在 |
|---|---|---|---|---|
| 1 | **Task Trace 提取** | 每 chunk 1 次产出结构化 Markdown，下游共享 | 每个提取器各扫一遍原始对话、过程态和知识态混存、Step 识别不稳定 | **MemOS TS**（3N+8 次 LLM，每个提取器独立走 pipeline）/ **AutoSkill**（Primary Input + Full Context 双层输入，每个提取器重复解析）/ **Mem0**（verbatim 逐步骤快照，无中间结构化层）/ **Hermes**（LLM 自主决定存什么，无结构化证据层）/ **pskoett SI**（agent 按 spec 手写 Markdown，线性膨胀无结构）|
| 2 | **滑动窗口 + 主题拼接** | 长任务被自动拼成一个 chunk 一次性消费 | 单窗口提取碎片化、长任务主题跨窗口丢失、LLM token 成本线性 | **MemOS TS**（固定 chunk 边界，任务数越多 LLM 越线性涨）/ **AutoSkill**（滑动 `messages[-6:]` 每轮扫一遍，无 chunk 拼接，长任务必碎）/ **Mem0**（无窗口，每次 API 全量处理，长对话 token 爆炸）/ **XSkill**（按 N=4 rollout 粒度，离线训练 OK 但在线无等价机制）|
| 3 | **分类 + 候选池 + 快慢车道** | `TSE / Fb / Tp` 三元 + `task / sub-task` 双级锚点 + 单表候选池 + 快慢两条升格通路 | 程序记忆粒度单一、硬约束生效慢、软经验无证据验证、候选/活跃层向量空间不一致 | **memory-tencentdb**（单一 `instruction` 类型，无粒度分层）/ **AutoSkill**（只 skill 一级，同步 add/merge/discard 无快慢分流）/ **Mem0**（无候选池，直接 ADD，无去重无演进）/ **ReMe**（向量 DB 单层，只老化无主动升格）/ **MemOS**（Chunk DUPLICATE + Skill add/upgrade 两套，每任务 5-9 次 LLM）/ **OpenViking**（SKIP 直接丢弃候选无审计） / **Hermes**（无 classifier / 无 hard trigger / 无 online evaluator）|
| 4 | **保证性提取（scheduler-driven）** | 定时器扫水位 + 6 层硬流水线，agent 不参与决策 | skill-triggered 系统依赖 agent 自觉调工具/自觉 append，用户反馈有**被漏记**的风险 | **Hermes**（agent 自调 `skill_manage` 工具，弱模型 / 长会话可能漏）/ **pskoett SI**（agent 自觉 append Markdown + `activator.sh` 每轮提醒，依赖 agent 读懂 573 行 spec）/ **Skill-Creator**（用户说"做个技能"才触发，默认不记）/ **Hermes GEPA**（离线 PR 评审，需人工介入）|
...
| pskoett SI | agent 按 spec 手写 Markdown | 线性膨胀无容量管理（`survey/Self-Improvement-Skill-Analysis.md:322`） |
...
- **TSE 是归纳出的套路**——单次 chunk 里 LLM 可能过拟合偶发现象，必须累积 ≥ 3 次 mention 才信任，且离线 worker 要读 source_trace 验证。所有 TSE 都走慢车道。
...
V6 是唯一同时做了"粒度分层 × 主文件-快照两层候选池 × 快慢双车道"的设计。三维度交叉后，所有组合（快慢 × hard/soft × task/sub-task）各走独立路径，复杂度都压在设计期，运行期规则极简。
...
这是区别于 **skill-triggered 自演进程序**（Hermes / pskoett SI / Skill-Creator 等）最根本的定位差异。只要用户在对话里说了"不对，改用 X"，**下一个水位扫描周期就会把它沉淀成 Fb/Tp**——agent 不需要意识到自己被纠正了，也不需要主动调工具。
...
| **pskoett SI** | agent 自觉 append Markdown + `activator.sh` 每轮提醒 | **中**——依赖 agent 读懂 573 行 SKILL.md 元 spec；每轮摊入 500-2k tokens |
...
8. **skill-triggered 系统 agent 漏记用户反馈**（Hermes / pskoett / Skill-Creator 都靠 agent 自觉；V6 定时器硬流水线不漏）
