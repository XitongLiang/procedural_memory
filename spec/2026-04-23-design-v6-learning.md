...
| `task` / `sub_task` | `procedural_concept` | 事实层：canonical 名字 + occurrence 计数 + (task 的) description | 跨 chunk 名字去重与统计；description 供 §7 查询拼 candidate.description。不被业务表 FK 引用，fb/tp/tse 表直接存同名 TEXT 列索引 |
...
Step 5  Chunk flush（Task Trace + Fb/TP + TSE）
          ...
          (B) Fb/Tp 提取（每 task 一次 LLM 调用，sub-task 作锚点）
              ...
              单层查重（procedural_candidates，(user_id, kind) 分区，
              v6.1 起取消 anchor 分区——LLM 生成 sub_task 名字不稳定）：
...
│     └── TSE 检索锚点（任务级，description = "{task}: {task_description}"）
...
```
 ...
 │  mem_record                                         │
 │   ├─ category=EXPERIENCE        + procedural_tse_meta  (description/content)
 │   ├─ category=FEEDBACK          + procedural_fb_meta   (description/content)
 │   └─ category=TOOL_PREFERENCE   + procedural_tp_meta   (description/content)
 ...
 │  anchor / content / description    │          │  每次 chunk 一条      │
...
```
combined_score = 0.80 × cos(embedding_a, embedding_b) + 0.20 × keyword_overlap(description_a, description_b)
```

- `embedding_*` 来自 `procedural_candidates_vec`，由 `description` 算出（§8.11）
- `keyword_overlap` 在两条 description 的分词结果上算 Jaccard
...
阈值常量：`PROCEDURAL_LEARN_JACCARD_THRESH`（`include/ltm_procedural_learn.h:43`），v6 值 **0.6f**。
...
升格时（§13 离线，可重复）：
  ⑥ candidate.mention_count 跨过新阈值 → 把 candidate 当前 playbook
     文本快照同步到 active（INSERT 新 mem_record 或 UPDATE 已有
     mem_record.content；v6.1 起 content 为编号 playbook 纯文本）
...
把本 chunk 总结的新经验合并进选中 candidate 的 playbook（整段替换）。

> **契约升级（v6.1，2026-04-24）**：输出从 JSON 改为**纯文本编号
> playbook**，`action` 由输出字节数隐式表达——空输出 = no_change，
> 非空 = enrich。

**输入**：
- 选中 candidate（含 task_name、task_description、当前 playbook 文本）
- 当前 Task Trace 完整 Markdown

**Prompt**（详见 prompts spec §7）：

```
[System]
你是一个任务经验合并专家。把本次 chunk 的 Task Trace 中可复用的经验合并进
已有 candidate 的 playbook。若无新增则输出空字符串。

## 合并原则（略，见 prompts spec §7）

## 输出形态判定（由输出字节数隐式表达）

- 有新增 / 质量提升 → 输出完整合并后的编号 playbook（整段替换）
- 无新增 / 已覆盖所有候选条目 → 输出空字符串（下游仅 mention++）

## 输出格式

严格编号 playbook（条与条之间空一行），不用 JSON / 不用 `-` 符号：
...
**写入**：UPDATE candidate
- `content` ← LLM 输出的 playbook 文本（非空 → 整段替换）/
  保留原值（空输出 → no_change）
- `mention_count++`
- `total_updates++`
- `last_seen_at = now`
...
**Prompt**（详见 prompts spec §6）：

> **契约升级（v6.1，2026-04-24）**：输出从 JSON 改为**纯文本编号
> playbook**，空输出 = 无可提取内容（跳过 INSERT）。

```
[System]
你是一个任务经验提取专家。从 Task Trace 中提取本次执行总结出的可复用
经验，输出编号 playbook 纯文本，直接作为新 candidate 初始内容。

## bullet 定义

- 每条 ≤ 50 字（`1.` 编号前缀不占字数额度），写"做法"不写"具体做了什么"
...
严格编号 playbook（条与条之间空一行）。示例：

1. 先 tail -n 200 看日志再决定是否全读

2. 改关键模块前先梳理上下游调用点

无可提取内容 → 输出空字符串（零字节），下游跳过创建 candidate。

[User]
{{task_trace}}
```

**写入**：`TseCandidateInsert(user_id, task=task_name, task_description, playbook=输出, trace_id)` → INSERT procedural_candidates(
- user_id, kind='tse',
- anchor=task_name,
- content=LLM 输出的 playbook 纯文本（非空时才 INSERT）,
- description="{task_name}: {task_description}"（写入时拼好固化）,
...
不在线触发。详见 §13 离线 worker：mention_count 跨过下一阈值时，把 candidate 的 description / content 当前态快照到 active（mem_record + procedural_tse_meta），candidate 本身保留继续累积。
...
CREATE TABLE procedural_candidates (
  ...
  content         TEXT    NOT NULL,
                  -- fb / tp: description 原文（单条规则文本）
                  -- tse:    编号 playbook 纯文本（v6.1 起；`1. ...\n\n2. ...`
                  --         多条 ≤ 50 字 bullet 拼接）
                  -- merge 时可覆盖（RECENCY BIAS 或 §8.10 合并 prompt 重写）
  description     TEXT    NOT NULL,
                  -- 用作 hybrid 召回的 embedding 输入，格式统一 "{anchor}: {xxx}":
                  -- fb / tp: "{sub_task}: {description_text}" 字面值（INSERT 时拼好，固化）
                  ...
                  --   跨分区时共享前缀 → 有相关性但不干扰
                  --   tse 每 task 一行，不重复
                  -- description 一次写入**不更新**（即使 content 改 / task_description 改也不动）
                  -- ——和 procedural_candidates_vec 的向量 1:1 绑定
...
**关键约束**：
- `anchor` / `content` / `description` 三列在 schema 层用泛化命名以容纳 3 类——业务代码按 kind 分支知道每列具体含义
- 同 `(user_id, kind)` 分区内才参与 hybrid 查重；跨分区永远不混淆
- `description` 一次写入后**不重算**——避免 candidate 频繁更新时 embedding 反复刷新；即使 `content` 后续 merge 覆盖，description 保留最初那份
- 配套向量表 `procedural_candidates_vec(candidate_id, embedding)`：embedding 用 description 作输入，§7.1 召回时按这个向量做 hybrid
...
- **主表 `mem_record`**：`memory_type = PROCEDURAL(2)`、`category = GSPD_CAT_EXPERIENCE(10)`、`tier = LEAF(2)`。`content` 字段由 `TseBuildMemRecordContent(description, content)` 拼成多行 Markdown（见 §7.3）。**embedding 输入**取 meta 表的 `description` 字段（与 candidate 的 `description` 字面一致——bullets 变化时只重写 content，不重算 embedding），写入 `mem_record_raw_vec_10`。
- **扩展表 `procedural_tse_meta`**（v6 精简后统一三栏）：

```sql
CREATE TABLE procedural_tse_meta (
  record_id       INTEGER PRIMARY KEY
                  REFERENCES mem_record(id) ON DELETE CASCADE,
  description     TEXT NOT NULL,                  -- "{task}: {task_description}" 固化串
                                                   -- 直接从 candidate.description 拷贝
  content         TEXT NOT NULL,                  -- 编号 playbook 纯文本（v6.1 起；
                                                   --   形如 `1. ...\n\n2. ...`，每条 ≤ 50 字）
                                                   -- 直接从 candidate.content 拷贝
  source_sessions TEXT                            -- JSON array，来源 session_id 列表
);
```

TSE 的锚是 task 整体——已包含在 `description` 的前缀里（"{task}: {...}"）。**候选池 `procedural_candidates` 单独保留 `anchor` 列**用于 pending 态的 SQL 结构化查询（如"列出某 task 下所有候选"）；active 层因为主要通过向量召回访问，不再单独存 `anchor`——需要时可从 description 前缀 parse 或 JOIN 候选池。`mention_count` / `hard_count` / `last_seen_at` 等计数都在 candidate 层累积，active 层只承载升格后的稳定快照。
...
| `content` | 编号 playbook 纯文本（v6.1 起；形如 `1. ...\n\n2. ...`，跨 chunk 累积） |
| `description` | `"{task}: {task_description}"`（创建时从 §8.1 join 出 task_description 固化） |
...
`UNIQUE(user_id, anchor) WHERE kind='tse'` 唯一索引保证：每个 user 的同一 task 在候选池里**最多 1 行**（区别于 fb/tp 一个 sub_task 下可有多条不同 content 的候选）。新 chunk 命中已有 task 候选时，把新 bullets merge 进现有 `content`，mention_count++。
...
- `TseCandidateLoad(candidate_id)` —— 拿单条 candidate（content 反序列化为 bullets list）
- `TseCandidateUpsertBullets(candidate_id, new_bullets)` —— §7.3 Enrich 写回 content + mention++（TSE 不动 hard_count）
- `TseCandidateInsert(user_id, task, task_description, bullets)` —— §7.4 Learn 新建（写入时拼 `description="{task}: {task_description}"`）
- `TsePromoteCandidate(candidate_id)` —— §13 离线 worker 调用，把 candidate 的 description / content 拷贝到 `mem_record + procedural_tse_meta`
...
主表 `content` 由 `FbBuildMemRecordContent(description, content)`
直接用 candidate 拷贝来的 `description`（已含 sub_task 前缀，形如
"{sub_task}: {rule}"）与 `content`（规则原文）合成展示串；同时 meta 表
的 `description` 字段作为 `mem_record_raw_vec_12` 的向量嵌入输入——
和 candidate 层的 `description`/vec 字面完全一致，升格时零重算。

```sql
CREATE TABLE procedural_fb_meta (
  record_id              INTEGER PRIMARY KEY
                         REFERENCES mem_record(id) ON DELETE CASCADE,
  description            TEXT NOT NULL,           -- "{sub_task}: {rule}" 固化串
                                                   -- 直接从 candidate.description 拷贝
  content                TEXT NOT NULL,           -- 规则原文
                                                   -- 直接从 candidate.content 拷贝
...
sub_task 信息已经内嵌在 `description` 前缀里（"{sub_task}: {rule}"）——active 层不再单独索引 sub_task 列，召回走向量路径。跨任务复用的结构化查询仍可通过 candidate 层的 `anchor` 列实现（pending pool 保留该列）。
...
主表 `content` 由 `TpBuildMemRecordContent(description, content)`
直接用 candidate 拷贝来的 `description`（已含 sub_task 前缀）与
`content`（规则原文）合成展示串；同时 meta 表 `description` 字段
作为 `mem_record_raw_vec_11` 的嵌入输入——和 candidate 层字面一致。
工具名（如 "Bash" / "Read"）由 LLM 写进 content（如 "做日志
检索时优先用 grep 而不是 cat"），不单独建列。
...
CREATE TABLE procedural_tp_meta (
  record_id       INTEGER PRIMARY KEY
                  REFERENCES mem_record(id) ON DELETE CASCADE,
  description     TEXT NOT NULL,                  -- "{sub_task}: {rule}" 固化串
  content         TEXT NOT NULL,                  -- 规则原文（含工具名）
  is_hard         INTEGER NOT NULL DEFAULT 1,
  source_sessions TEXT
);
```

与 §8.4 同构。sub_task 已内嵌在 description 前缀里，跨任务复用的结构化查询通过 candidate.anchor 实现。
...
查候选池（按 (kind, sub_task) 过滤后跑 hybrid score）
  ...
  │
  │   ── content 合并（§8.10）──
  │   if score ≥ 0.85: 直接按 RECENCY BIAS 用 c.content 覆盖，is_hard 取 OR
  │   else (0.50–0.85 小模型裁为 merge): 调 §8.10 合并 prompt
  │     → 更新 m.content / m.is_hard（description 固化不动，embedding 不重算）
  │
  ...
  └─ add（未找到）
      新建 pending candidate（
        kind='feedback' / 'tool_preference'，
        anchor=c.sub_task, content=c.content,
        description=f"{c.sub_task}: {c.content}"（INSERT 时拼好，固化），
        ...
        total_updates=1）
      同步算 embedding(description) → INSERT procedural_candidates_vec
...
def promote(cand):
    if cand.last_promoted_record_id is None:
        ...
        # 已有快照 → UPDATE 替换（不重算 embedding；description 已冻结）
...
|---|---|---|---|---|
| `procedural_candidates_vec` | `procedural_candidates` | `description`（§8.0.3） | INSERT candidate 时同步 | ❌（description 固化不更新） |
| `procedural_task_trace_vec` | `procedural_task_trace` | `"{task_name} {user_query}"`（trace 头部摘要） | INSERT trace 时同步 | ❌（trace 写入即终态） |
| `mem_record_raw_vec_{10,11,12}` | `mem_record`（active 层） | **与对应 candidate.description 字面一致**（见 §8.11.2） | §13 升格首次 INSERT 时 | ❌（后续 UPDATE 只改 content） |

#### 8.11.2 active 层 embedding 输入与 candidate 对齐

| Category | vec 表 | embedding 输入 | 对应 candidate.description |
|---|---|---|---|
| EXPERIENCE(10) = TSE | `mem_record_raw_vec_10` | `"{task}: {task_description}"` | **完全一致** |
| TOOL_PREFERENCE(11) = Tp | `mem_record_raw_vec_11` | `"{sub_task}: {rule}"` | **完全一致** |
| FEEDBACK(12) = Fb | `mem_record_raw_vec_12` | `"{sub_task}: {rule}"` | **完全一致** |
...
INSERT procedural_candidates (...)                 -- 行写入
  ↓
embedding = embedding_service.embed(description)   -- 同步调用
  ↓
...
- vec 表**从不 UPDATE**（description 不变 → embedding 不变）
...
| §7.3 EnrichTseBullets | 大模型 | 命中时输出合并后的编号 playbook 纯文本整段覆盖 `candidate.content`；空输出 = no_change（v6.1 契约） |
| §7.4 LearnTseBullets | 大模型 | 未命中时输出编号 playbook 纯文本作为新 candidate 初始内容；空输出 = 跳过（v6.1 契约） |
| **§8 候选池在线查重（灰区兜底）** | | |
...
**不调 LLM**（除 §13.2 合理性检查外）——bullets 列表做纯规则整理：按 candidate 原顺序编号、trim、去重相同文本；fb/tp 的 content 直接复制。

### 13.6 当前 v6 实现状态

**已实装**：在线 §6.1（Fb/Tp）、§7（TSE）写候选池；在线 is_hard 短路升格（§8.7-A）；离线 worker `RunPromoteWorkerOnce` 的完整升格逻辑（§13.1 扫描 + §13.2 证据检查 + §13.5 upsert active）；三类 active meta 表 + 向量表同步。

**尚未接入调度器**：`RunPromoteWorkerOnce` 的周期触发器（每日一次）暂无集成点，需要后续在 instance worker 层挂一个 timer 调用。当前阶段可手动调用触发升格。
