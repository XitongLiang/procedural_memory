...
用户消息输入
    ...
    ▼
[1] System Prompt 组装
    │  ── 注入 skill 引导指令 + 动态构建已有 skill 索引
    │
    ▼
[2] LLM 推理：识别 Skill 创建时机
    │  ── 由 system prompt 引导规则 + tool schema 触发条件驱动
    │  ── LLM 自主判断是否需要创建/更新 skill
    │
    ▼
[3] Skill 提取 & 创建
    │  ── LLM 调用 skill_manage(action="create") 工具
    │  ── 校验名称、分类、frontmatter、内容大小
    │  ── 原子写入 SKILL.md 文件
    │
    ▼
[4] 后处理：安全扫描
    ...
    ▼
[5] 缓存刷新 & 索引更新
    │  ── 新 skill 立即出现在下一次系统提示中
    │
    ▼
[6] Skill 加载 & 调用
       ── 用户 /skill-name 或 LLM 自动触发
       ── 读取 SKILL.md，组装激活消息注入上下文
```
...
[3] 基线约束校验
    │
    ▼
[4] 包装为 DSPy Module（SkillModule）
    │
    ▼
[5] GEPA 遗传进化优化
    │  ── 多代 mutation + selection
    │
    ▼
[6] 提取进化后 skill 文本
    │
...
1. **渐进式披露是共识**：AutoSkill/MemOS/Hermes 都用了三级加载，区别只在自动化程度
2. **证据溯源很关键**：用户输入为主要证据，agent 输出仅参考
