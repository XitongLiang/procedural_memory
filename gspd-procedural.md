...
#### 1.2.3 `context` 字段语义差异

| 类型 | context 应是什么 | 好例子 | 坏例子 |
|------|-----------------|--------|--------|
...
| **TSE** | **完整 description 句**（类似 skill description，描述"在什么任务情境下参考本经验"） | • "调试 FastAPI 后端项目，涉及 schema 生成、client 对接、接口测试等场景"<br>• "编写每周工作总结，需要汇总完成事项、进行中任务与遇到的问题"<br>• "推送代码到生产环境部署前，验证变更符合预期避免事故"<br>• "为新项目首次配置 CI/CD pipeline（GitHub Actions / GitLab CI 等）"<br>• "对已有代码模块进行重构或架构调整，需要保持行为兼容"<br>• "接入外部第三方 API 服务（支付、认证、消息推送等）"<br>• "进行数据库 schema 变更、数据迁移或索引优化"<br>• "分析和优化系统性能瓶颈（CPU / 内存 / IO / 查询慢）" | • "当用 curl 时"（是工具场景 → 应为 TP）<br>• "修改配置后"（是动作触发 → 应为 Fb）<br>• "FastAPI"（只是领域标签，非描述句） |

三类字段含义不同 → 检索查询的抽象层级必须分开 → **存储必须分开**（不能共享一张 context 向量表）。

**识别规则**：

- 若 context 可直接作为"**agent 执行某动作前/后**"的钩子 → 属于 **Fb**
- 若 context 是"**执行某类操作时需选工具**" → 属于 **TP**
- 若 context 是"**完成某类任务目标**" → 属于 **TSE**
...
| 检索精准度 | context 字段模糊（任务/动作/工具混杂） | context 按粒度分层，语义对齐 |
...
### 1.5 统一结构：`context + content`

三类在结构层面**完全统一**：

```
每条知识 = context + content
...
**意义**：
- **Schema 一致**：三类共用 `context + content` 数据模型，读写接口对称
- **提取模板一致**：LLM prompt 可以抽出共同骨架（"生成 `context: ...` + `content: ...`"），仅三类的 content 约束不同
- **展示一致**：UI 侧统一按 `[{context}] {content}` 呈现，不需要按类型写三套组件
...
| `context` | **Description / When to Use**：动作触发条件（类比 skill 的 description，告诉 agent 何时召回本条）| 以动作条件短语开头（示例："修改配置后" / "删除文件前" / "遇到 ECONNREFUSED 时" / "推送代码前" / "运行迁移前" / "调 API 返回非 200 时" / "回滚 commit 后" / "清理临时文件时"）|
...
| `context` | **Description / When to Use**：操作类型（类比 skill 的 description，告诉 agent 何时召回本条）| 操作类别短语（示例："搜代码时" / "管 Python 依赖时" / "读大文件时" / "查进程状态时" / "生成代码 diff 时" / "运行单元测试时" / "处理 JSON/YAML 时" / "查看 git 历史时"）|
...
```yaml
name: "FastAPI 开发"
context: "调试 FastAPI 后端项目，涉及 schema 生成、client 对接、接口测试等场景"
content:
...
```yaml
name: "周报生成"
context: "编写每周工作总结，需要汇总完成事项、进行中任务与遇到的问题"
content:
  - "1. 先查上周 TODO 列表和完成状态"
  - "2. 再扫本周会议纪要和关键讨论"
...
```yaml
name: "部署前检查"
context: "推送代码到生产环境部署前，验证变更符合预期避免事故"
content:
  - "1. 先跑 dry-run 看预期变更"
...
**提升任务完成度**：
窗口边界 ≈ 任务单元边界 → 后续提取的 context 天然贴合任务语义，不会把"部署"和"写文档"的经验混在一起。
...
**提升任务完成度**：
`topic_summary` 作为下游 Fb/TP/TSE 的 context 锚点 → **同类经验跨 session 归集到同一条**，dedup 命中率提升 → 记忆库更简洁、检索更精准。
...
**4. 下游三类提取共用同一 context 骨架**

Fb / TP / TSE 虽然 content 形态不同，但 context 字段都以 topic_summary 作参考背景：
...
hybrid 查重（`0.70·cos + 0.18·kw + 0.12·name`）里 cos 部分强依赖 context 稳定：

- 若没有主题锚点，context 字段会因 session 措辞不同而漂移，cos 分值抖动
- 有主题锚点 → 同主题的 context 都靠近同一向量簇 → dedup 阈值 0.85 的命中稳定
...
主题是一句人类可读的自然语言，**审计记忆库**时直接看 context 就知道"这条经验属于什么任务情境"。相比 tag id / tool 序列 hash，调试成本低一个量级。
...
| context 统一骨架 | Step 5-B/C 提取 | Fb/TP/TSE embedding 对齐 |
...
**3. 主题锚点贯穿三个 extractor，保证 context 对齐**
...
- Fb extractor 归纳出 context = "部署场景"
- TP extractor 归纳出 context = "后端开发"
- TSE extractor 归纳出 context = "接口调试"

同一任务的三条记忆 context **互不对齐**，检索时无法联动召回。Task Trace 把主题"钉死"，**三类共享同一情境坐标**。
...
| 三类 context 对齐 | 各自归纳，可能漂移 | 共享主题锚点 |
...
### 3.3 痛点 C：Context 粒度漂移导致检索/去重失败
...
| ReMe | context 向量按 `when_to_use` 建索引但**粒度混合**（有的句子有的短语）|
...
3. **三类 context 风格分档**（§1.2.3）—— Fb 动作短语 / TP 工具场景 / TSE 描述句，空间不互相挤占
