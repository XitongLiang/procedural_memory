...
**Step A — 多路径信号检测（从 Task Trace）**

扫描 Task Trace 中的 Step 列表，查找以下多路径信号：

```
1. Type = "correction" 的 Step（用户主动纠正或换方案）
2. Result 为 "失败" 后后续有 Result 为 "成功" 的 Step（agent 自行换方案恢复）
3. User Query 中包含"换一种"/"重新"/"改用"/"换成"等切换意图词
```

命中 ≥ 1 条 → 进入 Step B。
未命中 → 跳过，走常规 §7.2 路线。

**Step B — 执行路径切分（基于 Task Trace）**

以**多路径信号所在 Step** 为切分点，将 Task Trace 的 Step 列表划分为两段：

```
路径 A = TaskTrace[切分点前所有 Step]（第一次执行路径）
路径 B = TaskTrace[切分点后所有 Step]（第二次执行路径）
```

验证两段各自都包含实质性动作（工具调用 Step 或 agent 产出 Step）：
- 两段都有效 → 确认存在多路径，标记 `has_alternative_paths=true`
- 任一段无效 → 降级为常规单路径处理

**Step C — 结果优劣推断（零 LLM 辅助）**

扫描切分点后的 Task Trace Step，推断哪条路径更受认可：

```
正面信号（路径 B 更优）: Step 中有 "这个好"/"可以"/"完美了"/"对了"/"就这个"
负面信号（路径 A 更优或都差）: Step 中有 "还是不行"/"也不对"/"算了"/"回到之前的"
```
...
**输入**：
```
Path A Trace        ← 切分点前的 Task Trace Step 列表（第一次执行路径）
Path B Trace        ← 切分点后的 Task Trace Step 列表（第二次执行路径）
...
### 1) 证据来源
- 以 Path A Trace 和 Path B Trace 为主要证据（已结构化，Step-by-step 清晰）
- Task Trace 中的 User Query 作为任务锚点
- 必须有两条路径都产生了实质性执行动作（工具调用 Step 或 agent 产出 Step）才能提取
  ...
  for Wi in windows:
    ...
    └─ [Step 4] 主题拼接状态机 DecideChunkAction
         ...
         flush → ChunkFlushV4(chunk):
           ...
           │
           (C) TSE 提取（从 Task Trace 提取）
               ExtractChunkOverview(Task Trace) → 多路径检测（零 LLM）
               有多路径 → ComparativeLearnTaskExperience(Task Trace 切分)
